...
1. What is the procedure for logging into Hoffman2? Since I am an Apple user, I will initially open a terminal. Then I will enter my username followed by "@hoffman2.idre.ucla.edu". Then enter you assigned password.
...
1. You can use two flags at the same time. What does the command ls do when used with the -l flag? What about if you use both the -l and the -h flag? The command "-l flag" adds more information about the file sizes and data edited while listing the files. If one uses both the "-l" and "-h" it will print the same information while putting it in a format for it to be easier to read.
